...
type HelloServiceClient interface {
	...
	Server(ctx context.Context, in *String, opts ...grpc.CallOption) (HelloService_ServerClient, error)
}
...
func (c *helloServiceClient) Server(ctx context.Context, in *String, opts ...grpc.CallOption) (HelloService_ServerClient, error) {
	stream, err := c.cc.NewStream(ctx, &HelloService_ServiceDesc.Streams[1], "/proto.HelloService/Server", opts...)
	if err != nil {
		return nil, err
	}
	x := &helloServiceServerClient{stream}
...
type HelloService_ServerClient interface {
	Recv() (*String, error)
	grpc.ClientStream
}

type helloServiceServerClient struct {
	grpc.ClientStream
}

func (x *helloServiceServerClient) Recv() (*String, error) {
	m := new(String)
	if err := x.ClientStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
type HelloServiceServer interface {
	...
	Server(*String, HelloService_ServerServer) error
...
func (UnimplementedHelloServiceServer) Server(*String, HelloService_ServerServer) error {
...
func _HelloService_Server_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(String)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(HelloServiceServer).Server(m, &helloServiceServerServer{stream})
}

type HelloService_ServerServer interface {
	Send(*String) error
	grpc.ServerStream
}

type helloServiceServerServer struct {
	grpc.ServerStream
}

func (x *helloServiceServerServer) Send(m *String) error {
	return x.ServerStream.SendMsg(m)
}
...
var HelloService_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		{
			StreamName:    "Server",
			Handler:       _HelloService_Server_Handler,
			ServerStreams: true,
		},
	},
