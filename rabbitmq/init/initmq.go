...
func (r *RabbitMQ) PublishDelay(message string) {
	...
	amqp.Publishing{
		ContentType: "text/plain",
		Body:        []byte(message),
		Expiration:  "1000", //设置1秒后过期
	})
}

func (r *RabbitMQ) RecieveDelay(){
	r.channel.ExchangeDeclare(
		r.Exchange,
		"direct",
		true,
		...
		false,
		nil)
	delayQueue,_:=r.channel.QueueDeclare(
		"delay",
		true,
		false,
		false,
		...
		amqp.Table{
			...
			"x-dead-letter-routing-key": "common",
		})
	common,_:=r.channel.QueueDeclare(
		"common",
		true,
		false,
		false,
